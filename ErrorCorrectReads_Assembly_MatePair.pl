...
#if ($args{PAIRED_READS_A_IN} && $args{PAIRED_READS_B_IN}) {
#$lg_run_dir= "/rhome/cjinfeng/BigData/00.RD/ALLPATH_LG/HEG4_RAW.3/data/run";
if (!$ARGV[0]){
   print "INPUT not specified!!\n";
   print "perl $0 /rhome/cjinfeng/BigData/00.RD/ALLPATH_LG/HEG4_RAW.3/data/run\n";
   print "perl $0 /rhome/cjinfeng/BigData/00.RD/ALLPATH_LG/HEG4_RAW.3/data/run 1\n";
   exit(0);
}
$lg_run_dir = $ARGV[0];
$output_dir= ".";
...
# ---- Run SplitReadsByLibrary
  ...
  $cmd = ("$FindBin::Bin/SplitReadsByLibrary" .
	  " READS_IN=$lg_run_dir/jump_reads_ec" .
	  " SPLIT_PAIRS=True" .
	  " QUALS=FALSE" );
  #run_or_die($cmd);
  my @fastb_A = glob("$lg_run_dir/jump_reads_ec.*.A.fastb");  
  my @fastb_B = glob("$lg_run_dir/jump_reads_ec.*.B.fastb");
  for (my $i =0 ; $i < @fastb_A; $i++){
      my $head = basename($fastb_A[$i], ".fastb");
      fastb_to_fastq("$lg_run_dir/$head", "$output_dir/$head", $phred_encoding); 
  }
  for (my $i =0 ; $i < @fastb_B; $i++){
      my $head = basename($fastb_B[$i], ".fastb");
      fastb_to_fastq("$lg_run_dir/$head", "$output_dir/$head", $phred_encoding);
  }
...
#		 "$output_dir$fastq_head_out.paired.B", $phred_encoding);
  if (-e "$lg_run_dir/jump_reads_ec.unpaired.fastb") {
      fastb_to_fastq("$lg_run_dir/frag_reads_corr.unpaired", "$output_dir/frag_reads_corr.unpaired", $phred_encoding);
  }
  if ($ARGV[1]) { ### if set 1 to fill fragment reads
    fastb_to_fastq("$lg_run_dir/filled_reads",
		   "$output_dir/filled_reads", $phred_encoding);
  }
