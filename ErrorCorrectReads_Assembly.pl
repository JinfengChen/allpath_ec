...
# ---- Run SplitReadsByLibrary
  ...
  $cmd = ("$FindBin::Bin/SplitReadsByLibrary" .
	  ...
	  " QUALS=True" );
  run_or_die($cmd);
  my @fastb_A = glob("$lg_run_dir/frag_reads_corr.*.A.fastb");  
  my @fastb_B = glob("$lg_run_dir/frag_reads_corr.*.B.fastb");
  for (my $i =0 ; $i < @fastb_A; $i++){
      my $head = basename($fastb_A, ".fastb");
      fastb_to_fastq("$lg_run_dir/$head", "$output_dir/$head", $phred_encoding); 
  }
  for (my $i =0 ; $i < @fastb_B; $i++){
      my $head = basename($fastb_B, ".fastb");
      fastb_to_fastq("$lg_run_dir/$head", "$output_dir/$head", $phred_encoding);
  }
